...
Usage:
    python scripts/compute_advanced_gds_features.py          # Dry-run (plan only)
    python scripts/compute_advanced_gds_features.py --execute  # Actually compute
"""
...
def get_gds_client(database: str = None):
    ...
    if not GDS_AVAILABLE:
        raise ImportError("graphdatascience not available. Install with: pip install graphdatascience")
...
def compute_tech_adoption_prediction(gds, driver, database: str = None):
    ...
    try:
        ...
        print(f"   ✓ Created graph: {result['nodeCount']} nodes, {result['relationshipCount']} relationships")
        ...
        with driver.session(database=database) as session:
            ...
            print(f"   Progress updates every 50 technologies...")
            ...
            for idx, (tech_id, tech_name) in enumerate(technologies, 1):
                try:
                    ...
                    if idx % 50 == 0 or idx == total_techs:
                        ...
                        print(f"   Progress: {idx}/{total_techs} technologies ({idx*100/total_techs:.1f}%) | "
        ...
        with driver.session(database=database) as session:
            ...
            if isinstance(similarity_result, pd.DataFrame):
                ...
                if not (col1 and col2 and sim_col):
                    print(f"   ✗ Error: Could not identify columns. Available: {list(similarity_result.columns)}")
                    raise ValueError(f"Unexpected DataFrame columns: {list(similarity_result.columns)}")
                ...
                for _, row in similarity_result.iterrows():
                    ...
                    """, node_id=node_id1).single()
                    tech2 = session.run("""
                        MATCH (t:Technology)
                        WHERE id(t) = $node_id
                        RETURN t.name AS name
                    """, node_id=node_id2).single()
                    ...
                    if tech1 and tech2:
                        ...
                        session.run("""
                            ...
                            SET r.similarity = $similarity,
                                r.metric = 'JACCARD',
                                r.computed_at = datetime()
                        """, tech1=tech1["name"], tech2=tech2["name"], similarity=similarity)
                ...
                for row in similarity_result:
                    if isinstance(row, dict):
                        ...
                        similarity = float(row.get("similarity", row.get("score", row.get("weight"))))
                    else:
                        node_id1 = int(row[0])
                        node_id2 = int(row[1])
...
def main():
    ...
    if args.execute:
        ...
        log_file = log_dir / f"compute_advanced_gds_features_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
    ...
    try:
        ...
        if not args.execute:
            ...
            print("   - Note: This could be flipped to answer 'which techs will this domain adopt?'")
            print()
            ...
            print()
            print("=" * 70)
            print("To execute, run: python scripts/compute_advanced_gds_features.py --execute")
            print("=" * 70)
            return
