...
def create_embedding(client: OpenAI, text: str, model: str = EMBEDDING_MODEL) -> Optional[List[float]]:
...
def main():
    ...
    if not args.execute:
        ...
        logger.info(f"  ~${entries_with_descriptions * 0.00002:.2f} for {entries_with_descriptions} embeddings")
        logger.info("")
    ...
    # Statistics
    entries_with_embeddings = sum(1 for entry in enriched_data if entry.get("description_embedding"))
    logger.info("=" * 80)
