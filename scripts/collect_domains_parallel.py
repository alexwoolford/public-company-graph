...
_rate_limits = {
    "sec": {"lock": Lock(), "last_call": 0, "min_interval": 1.0 / 9.0},  # 9 req/sec
    "finviz": {"lock": Lock(), "last_call": 0, "min_interval": 0.5},  # 2 req/sec
    "finnhub": {"lock": Lock(), "last_call": 0, "min_interval": 1.0},  # 1 req/sec
    "yfinance": {"lock": Lock(), "last_call": 0, "min_interval": 0.0},  # No explicit limit
}
...
def get_domain_from_yfinance(ticker: str, company_name: str = "") -> DomainResult:
    """Get domain from yfinance (high confidence source)."""
    ...
    try:
        ...
        if website:
            ...
            if domain and not is_infrastructure_domain(domain):
                return DomainResult(domain, "yfinance", 0.9, {"raw_website": website})
...
def get_domain_from_finnhub(ticker: str) -> DomainResult:
    """Get domain from Finnhub (low confidence, incomplete coverage)."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if weburl:
                ...
                if domain and not is_infrastructure_domain(domain):
                    return DomainResult(domain, "finnhub", 0.6)
    except Exception as e:
        if _logger:
...
def main():
    ...
    log_file = log_dir / f"collect_domains_parallel_{timestamp}.log"
    ...
    output_file = Path("data/public_company_domains_parallel.json")
    domain_list_file = Path("data/public_company_domains_parallel_list.txt")
    ...
    # Process in parallel
    max_workers = 20  # Process 20 companies concurrently
                ...
                try:
                    ...
                    if result:
                        ...
                        if len(results) % 100 == 0:
                            ...
                            _logger.info(f"Progress: {len(results)} companies found ({rate:.1f} companies/sec)")
                except TimeoutError:
                    ...
                    if processed_count % save_interval == 0:
                        ...
                        _logger.info(f"Incremental save: {len(results)} companies saved to {output_file}")
