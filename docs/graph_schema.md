...
**Data Source**: SQLite database (`data/url_checker.db`) containing domain status checks and technology detection results. The database is generated by [`domain_status`](https://github.com/alexwoolford/domain_status), a Rust-based tool for high-performance concurrent URL checking and technology fingerprinting.
...
The graph is loaded from `data/url_checker.db` using `scripts/bootstrap_graph.py`:
